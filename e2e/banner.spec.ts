...
test.describe("Critical Requests Banner", () => {
  test("should show critical banner for admin users", async ({ page }) => {
  ...
  test("should not show critical banner for requester users", async ({ page }) => {
    await login(page, "requester@requesthub.dev", "requester123");
    const banner = page.getByTestId("critical-banner");
    await expect(banner).not.toBeVisible();
  });

  test("should not show critical banner for reviewer users", async ({ page }) => {
